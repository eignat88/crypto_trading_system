#!/usr/bin/env python3
"""Backtest Scalping Momentum strategy.

Usage:
    python scripts/backtest_scalping.py
    python scripts/backtest_scalping.py --balance 1000
    python scripts/backtest_scalping.py --symbol ETHUSDT
"""
...
async def run_backtest(balance: Decimal, symbol: str) -> None:
    """Run backtest with Scalping Momentum strategy."""
    import asyncpg
    from app.config.settings import Settings
    from app.backtest.backtest_engine import BacktestConfig, BacktestEngine
    from app.strategies.scalping_momentum import ScalpingMomentumConfig, ScalpingMomentumStrategy
    ...
    print("=" * 60)
    print(f"BACKTEST: Scalping Momentum — {symbol}")
    print("=" * 60)
    ...
    # Create strategy
    config = ScalpingMomentumConfig()
    strategy = ScalpingMomentumStrategy([symbol], config=config)
    ...
    report_path = PROJECT_ROOT / "artifacts" / f"backtest_{symbol.lower()}_scalping.json"
