#!/usr/bin/env python3
"""Backtest BTC Trend Pullback DCA v1 strategy.

Usage:
    python scripts/backtest_btc_strategy.py
    python scripts/backtest_btc_strategy.py --balance 5000
"""
...
async def run_backtest(balance: Decimal) -> None:
    """Run backtest with BTC Trend Pullback DCA v1."""
    import asyncpg
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch("""
            ...
            WHERE i.symbol = 'BTCUSDT' AND c.interval_code = '1h' AND c.is_valid = true
    ...
    print("=" * 60)
    print("BACKTEST: BTC Trend Pullback DCA v1")
    print("=" * 60)
    ...
    # Create strategy
    config = BTCTrendPullbackDCAConfig()
    strategy = BTCTrendPullbackDCAStrategy(["BTCUSDT"], config=config)
    ...
    # DCA level analysis
    dca_signals = [s for s in result.signals if s.metadata.get("dca_level")]
    print(f"\nDCA entries: {len(dca_signals)}")
    for level in range(1, 5):
        level_signals = [s for s in dca_signals if s.metadata.get("dca_level") == level]
        if level_signals:
            avg_price = sum(s.price for s in level_signals) / len(level_signals)
            print(f"  Level {level}: {len(level_signals)} entries, avg price: {avg_price:.0f}")

    # Save report
    report = {
        "strategy": "BTCTrendPullbackDCA_v1",
        "balance": str(balance),
        "candles": len(candles),
        ...
        "dca_entries": len(dca_signals),
    }
    report_path = PROJECT_ROOT / "artifacts" / "backtest_btc_trend_pullback_dca_v1.json"
...
def main() -> None:
    ...
    asyncio.run(run_backtest(args.balance))
