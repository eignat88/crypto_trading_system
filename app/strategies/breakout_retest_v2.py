...
class BreakoutRetestV2Strategy(BreakoutRetestStrategy):
    ...
    def __init__(self, symbols: list[str], config: BreakoutRetestV2Config | None = None) -> None:
        frozen = config or BreakoutRetestV2Config()
        if frozen.resistance_lookback_bars != RESISTANCE_LOOKBACK_BARS:
            raise ValueError(f"resistance_lookback_bars is frozen at {RESISTANCE_LOOKBACK_BARS} for v2")
        if frozen.retest_timeout_bars != RETEST_TIMEOUT_BARS:
            raise ValueError(f"retest_timeout_bars is frozen at {RETEST_TIMEOUT_BARS} for v2")
        if frozen.failure_detection_age_bars != FAILURE_DETECTION_AGE_BARS:
            raise ValueError(f"failure_detection_age_bars is frozen at {FAILURE_DETECTION_AGE_BARS} for v2")
        if frozen.failure_watch_max_bars != FAILURE_WATCH_MAX_BARS:
            raise ValueError(f"failure_watch_max_bars is frozen at {FAILURE_WATCH_MAX_BARS} for v2")
    ...
    def _new_symbol_state() -> dict[str, Any]:
        return {
            "phase": "IDLE", "breakout": None, "recent_highs": [],
            "last_observed_time": None, "last_prior_resistance": None,
            "position_state": None, "position_entry_fill_time": None,
            "position_entry_price": None, "position_breakout_level": None,
            "position_age_bars": 0, "last_position_observed_time": None,
            "failure_watch_used": False, "failure_watch_start_time": None,
            "failure_watch_bars": 0, "failure_watch_trigger_close": None,
            "failure_watch_trigger_ema20": None, "failure_watch_trigger_ema50": None,
            "failure_watch_trigger_breakout_level": None,
            "failure_watch_resolution": None, "failure_watch_resolution_time": None,
            "transition_events": [],
        }

    def _append_event(self, symbol_state: dict[str, Any], *, event: str, timestamp: datetime, metadata: dict[str, Any] | None = None) -> None:
        events = list(symbol_state.get("transition_events") or [])
        events.append({"event": event, "timestamp": timestamp.isoformat(), "strategy": self.name,
                       "parameters_version": self.config.parameters_version, "metadata": metadata or {}})
    ...
    def _observe_position_candle(self, symbol_state: dict[str, Any], timestamp: datetime) -> bool:
    ...
    def _mark_watch_hard_exit(self, symbol_state: dict[str, Any], *, signal: Signal, timestamp: datetime) -> None:
        if symbol_state.get("position_state") != "FAILURE_WATCH":
            return
        reason_map = {
            "Regime changed to TREND_DOWN": "TREND_DOWN", "Max holding period reached": "MAX_HOLDING",
            "Trailing stop hit": "TRAILING", "Take profit hit": "TAKE_PROFIT", "Stop loss hit": "STOP_LOSS",
        }
        ...
        self._append_event(symbol_state, event=f"FAILURE_WATCH_RESOLVED_BY_{resolution}", timestamp=timestamp,
                           metadata={"reason": signal.reason})

    def _build_watch_timeout_signal(self, *, candle: dict[str, Any], indicators: dict[str, Any], position: dict[str, Any], symbol_state: dict[str, Any]) -> Signal:
        ...
        return Signal(action="close", symbol=symbol, price=close, quantity=quantity, timestamp=timestamp,
                      reason="Failure watch timeout without structural recovery", strategy=self.name,
                      parameters_version=self.config.parameters_version, indicators=indicators,
                      regime=None if regime is None else str(regime),
                      metadata={"position_age_bars": int(symbol_state["position_age_bars"]),
    ...
    def should_enter(self, candle: dict[str, Any], indicators: dict[str, Any], portfolio_state: dict[str, Any]) -> Signal | None:
        symbol_state = self._symbol_state(str(candle["symbol"]))
        if not bool(portfolio_state.get("has_position", False)) and symbol_state.get("position_state") is not None:
            self._reset_position_management(symbol_state)
        return super().should_enter(candle, indicators, portfolio_state)

    def should_exit(self, candle: dict[str, Any], indicators: dict[str, Any], position: dict[str, Any]) -> Signal | None:
        symbol = str(candle["symbol"])
        timestamp = candle["open_time"]
        if not isinstance(timestamp, datetime):
            raise TypeError("candle.open_time must be datetime")
        ...
        if symbol_state.get("position_state") is None:
            raise ValueError(f"Open portfolio position for {symbol} has no v2 position-management state")
        if not self._observe_position_candle(symbol_state, timestamp):
            return None

        close = self._as_decimal(candle["close"])
        ema20_raw, ema50_raw = indicators.get("ema_20"), indicators.get("ema_50")
        ...
        if position_state == "NORMAL_POSITION":
            ...
            ema20, ema50 = self._as_decimal(ema20_raw), self._as_decimal(ema50_raw)
            ...
            if close < ema20 and close < ema50 and close < breakout_level:
                ...
                symbol_state["failure_watch_resolution"] = None
                symbol_state["failure_watch_resolution_time"] = None
                self._append_event(symbol_state, event="FAILURE_WATCH_STARTED", timestamp=timestamp,
                                   metadata={"position_age_bars": int(symbol_state["position_age_bars"]),
                                             "close": str(close), "ema20": str(ema20), "ema50": str(ema50),
                                             "breakout_level": str(breakout_level),
                                             "regime": None if indicators.get("regime") is None else str(indicators.get("regime"))})
            return None
        ...
        if recovered:
            ...
            symbol_state["failure_watch_resolution_time"] = timestamp.isoformat()
            self._append_event(symbol_state, event="FAILURE_WATCH_RECOVERED", timestamp=timestamp,
                               metadata={"failure_watch_bars": completed_watch_bars})
            # Deactivate the counter but retain trigger snapshot/start time for audit
            # until the actual position closes.
            symbol_state["failure_watch_bars"] = 0
            return None

        if int(symbol_state["failure_watch_bars"]) >= self.config.failure_watch_max_bars:
            signal = self._build_watch_timeout_signal(candle=candle, indicators=indicators, position=position,
                                                      symbol_state=symbol_state)
            if symbol_state.get("failure_watch_resolution") != "TIMEOUT_SIGNAL":
                symbol_state["failure_watch_resolution"] = "TIMEOUT_SIGNAL"
                symbol_state["failure_watch_resolution_time"] = timestamp.isoformat()
                self._append_event(symbol_state, event="FAILURE_WATCH_TIMEOUT_SIGNAL", timestamp=timestamp,
                                   metadata={"failure_watch_bars": int(symbol_state["failure_watch_bars"])})
            return signal
        return None

    def should_add_dca(self, candle: dict[str, Any], indicators: dict[str, Any], position: dict[str, Any]) -> Signal | None:
    ...
    def on_fill(self, signal: Signal, fill: Fill) -> None:
        ...
        if fill.side == "buy":
            ...
            symbol_state["position_age_bars"] = 0
            symbol_state["failure_watch_used"] = False
            self._append_event(symbol_state, event="POSITION_NORMAL_STARTED", timestamp=fill.timestamp,
                               metadata={"entry_price": str(fill.price), "breakout_level": str(level)})
            return
        if fill.side == "sell":
            self._append_event(symbol_state, event="POSITION_CLOSED", timestamp=fill.timestamp,
