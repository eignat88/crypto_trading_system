    ...
    *,
    ...
    for row in fill_rows:
        ...
        if first_td is not None:
            td_hours = Decimal(str((first_td - signal_time).total_seconds())) / Decimal("3600")

        ema20 = _d(candle.get("ema_20"))
        ema50 = _d(candle.get("ema_50"))
        ema200 = _d(candle.get("ema_200"))
        atr = _d(candle.get("atr"))
        ...
        records.append(
            EntryQualityRecord(
                ...
                entry_signal_price=_d(entry_signal.get("price")) or close,
                entry_fill_price=entry_fill_price,
                weighted_entry_price=weighted,
                dca_count=max(0, buy_count - 1),
                rsi=_d(candle.get("rsi")),
                ema_20=ema20,
                ema_50=ema50,
                ema_200=ema200,
                close_to_ema20=_ratio(close, ema20),
                close_to_ema50=_ratio(close, ema50),
                close_to_ema200=_ratio(close, ema200),
                ema20_slope_10=_slope(candle_rows, idx, "ema_20"),
                ema50_slope_10=_slope(candle_rows, idx, "ema_50"),
                ema200_slope_10=_slope(candle_rows, idx, "ema_200"),
                atr=atr,
                atr_pct=(atr / close if atr is not None and close != 0 else None),
                volatility=_d(candle.get("volatility")),
                regime_confidence=_d(candle.get("regime_confidence")),
                trend_up_age_bars=age,
                trend_up_age_censored=age_censored,
                time_to_trend_down_hours=td_hours,
                trend_down_before_exit=first_td is not None,
                exit_time=fill_time,
...
def summarize_group(group: str, records: list[EntryQualityRecord]) -> EntryQualityGroupSummary:
    ...
    td_times = [r.time_to_trend_down_hours for r in selected]
    return EntryQualityGroupSummary(
        group=group,
        trades=len(selected),
        average_pnl=_avg([r.realized_pnl for r in selected]) or Decimal("0"),
        average_rsi=_avg([r.rsi for r in selected]),
        median_rsi=_median([r.rsi for r in selected]),
        average_close_to_ema20=_avg([r.close_to_ema20 for r in selected]),
        average_close_to_ema50=_avg([r.close_to_ema50 for r in selected]),
        average_close_to_ema200=_avg([r.close_to_ema200 for r in selected]),
        average_ema20_slope_10=_avg([r.ema20_slope_10 for r in selected]),
        average_ema50_slope_10=_avg([r.ema50_slope_10 for r in selected]),
        average_ema200_slope_10=_avg([r.ema200_slope_10 for r in selected]),
        average_atr_pct=_avg([r.atr_pct for r in selected]),
        average_volatility=_avg([r.volatility for r in selected]),
        average_regime_confidence=_avg([r.regime_confidence for r in selected]),
        average_trend_up_age_bars=_avg(ages) or Decimal("0"),
        median_trend_up_age_bars=_median(ages) or Decimal("0"),
        trend_down_before_exit=sum(r.trend_down_before_exit for r in selected),
        average_time_to_trend_down_hours=_avg(td_times),
    )


async def build_entry_quality_diagnostics(
    run_id: UUID,
    ...
    async with session_factory() as session:
        result = await session.execute(text("""
            ...
            FROM mart.backtest_run WHERE run_id = :run_id
        """), {"run_id": run_id})
        ...
        fills = await session.execute(text("""
            SELECT f.sequence_no, f.side, f.quantity, f.price, f.commission, f.fill_time,
                   o.payload -> 'signal' AS signal
            FROM mart.backtest_fill f
            JOIN mart.backtest_order o ON o.run_id=f.run_id AND o.order_id=f.order_id
        ...
        candles = await session.execute(text("""
            SELECT c.open_time, c.close_price,
                   e20.indicator_value AS ema_20,
                   e50.indicator_value AS ema_50,
                   e200.indicator_value AS ema_200,
                   rsi.indicator_value AS rsi,
                   atr.indicator_value AS atr,
                   vol.indicator_value AS volatility,
            ...
            WHERE i.exchange_name=:exchange AND i.symbol=:symbol AND c.interval_code=:interval
              AND c.open_time >= :start AND c.open_time < :end AND c.is_valid=true
            ORDER BY c.open_time
        """), {
            "exchange": run["exchange_name"], "symbol": run["symbol"],
            "interval": run["interval_code"], "start": run["period_start"], "end": run["period_end"],
        })
        candle_rows = [dict(r) for r in candles.mappings().all()]

    records = reconstruct_entry_quality(
        run_id=run_id, symbol=str(run["symbol"]), fill_rows=fill_rows, candle_rows=candle_rows
    )
    return EntryQualityReport(
        ...
        trend_down_losses=summarize_group("TREND_DOWN_LOSS", records),
        winners=summarize_group("WINNER", records),
