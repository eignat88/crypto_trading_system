...
- Общий автоматический запуск после каждой загрузки свечей подключён через флаг `calculate_indicators`
- Добавлен CLI-флаг `--skip-indicators` для пропуска расчёта при необходимости
...
- Реализован ETL-агрегаций из DDS в MART (`sql/006_dds_to_mart_etl.sql`)
- Добавлен CLI-загрузчик `scripts/load_mart.py` с поддержкой полного и частичного пересчёта
- Реализована идемпотентность через ON CONFLICT DO UPDATE
- Добавлен журнал запусков в `mart.etl_run`
